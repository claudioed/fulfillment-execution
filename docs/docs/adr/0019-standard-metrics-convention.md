...
title: 0019. Standard metrics convention across the fleet
sidebar_label: 0019. Standard metrics convention
description: ADR 0019 — a fleet-wide Tier 1 (mandatory, identical) / Tier 2 (business, convention-only) split for OTel metrics, so instrument names and attribute keys are predictable across services without reading each one's source. fulfillment-execution is already compliant.
---

# 0019. Standard metrics convention across the fleet
...
**This repository's status:** `fulfillment-execution` is already compliant
with this ADR as written. It calls `telemetry.Setup`, wires the three
`otelchi` middleware lines ahead of the request logger, exposes
`GET /healthz`, and its business counters —
`fulfillment.tasks.claimed` / `fulfillment.tasks.completed` (attr
`task.type`) — already follow the Tier 2 naming shape below. One structural
note: this repo's telemetry lives at `internal/observability/` rather than
`internal/adapters/outbound/telemetry/` as in most other fleet repos. That
is a file-layout difference only — this ADR governs instrument naming and
...
- **Placement:** the instrument lives in the SAME adapter tier as
  ...
  `inventory-storage`'s `ReservationMetrics` already establishes. In this
  repository that adapter tier is `internal/observability/` rather than
  `internal/adapters/outbound/telemetry/` — a naming/location difference
  that predates this ADR and is not something this decision requires
  changing.
...
`fulfillment-execution` requires no changes under this decision: its
existing `telemetry.Setup`, `otelchi` wiring, and
`fulfillment.tasks.claimed` / `fulfillment.tasks.completed` counters
(attr `task.type`) are cited above as compliant examples of the shape
other services are being brought into line with.
...
- For `fulfillment-execution` specifically: no code changes result from
  this ADR. It documents an already-compliant state, and records the
  `internal/observability/` vs. `internal/adapters/outbound/telemetry/`
  layout difference as an accepted, non-blocking deviation.

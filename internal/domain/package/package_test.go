...
func newPackage() *pack.Package {
	return pack.New(shared.PackageId("p1"), shared.OrderRef("order-1"), "task-1", false, false)
}
...
func TestRehydrate_ReconstructsPersistedState(t *testing.T) {
	p := pack.Rehydrate(shared.PackageId("p2"), shared.OrderRef("order-2"), "task-1", pack.Sealed, []string{"sku-1", "sku-2"}, false, nil, false)
...
func TestNew_FragileHandling_RoundTrips(t *testing.T) {
	p := pack.New(shared.PackageId("p1"), shared.OrderRef("order-1"), "task-1", true, false)
...
func TestRehydrate_FragileHandling_RoundTrips(t *testing.T) {
	p := pack.Rehydrate(shared.PackageId("p2"), shared.OrderRef("order-2"), "task-1", pack.Sealed, []string{"sku-1"}, true, nil, false)
	if !p.FragileHandling() {
...
func TestNew_GiftWrapRequested_RoundTrips(t *testing.T) {
	p := pack.New(shared.PackageId("p1"), shared.OrderRef("order-1"), "task-1", false, true)
...
func TestRehydrate_GiftWrapRequested_RoundTrips(t *testing.T) {
	p := pack.Rehydrate(shared.PackageId("p2"), shared.OrderRef("order-2"), "task-1", pack.Sealed, []string{"sku-1"}, false, nil, true)
	if !p.GiftWrapRequested() {
...
func TestNew_FragileHandlingAndGiftWrapRequested_AreIndependent(t *testing.T) {
	...
	p2 := pack.New(shared.PackageId("p2"), shared.OrderRef("order-2"), "task-1", false, true)
...
func TestScanItemWithClass_HazardClassesRoundTripThroughRehydrate(t *testing.T) {
	p := pack.Rehydrate(shared.PackageId("p2"), shared.OrderRef("order-2"), "task-1", pack.Sealed, []string{"sku-1"}, false, []int{3}, false)
	got := p.ScannedHazardClasses()
...
func TestSortLane_HazmatBeatsFragile(t *testing.T) {
	p := pack.New(shared.PackageId("p1"), shared.OrderRef("order-1"), "task-1", true, false) // fragile
...
func TestSortLane_HazmatOnly(t *testing.T) {
	p := pack.New(shared.PackageId("p1"), shared.OrderRef("order-1"), "task-1", false, false)

...
// commits together or not at all (ADR 0020).
...
// a ctx that already carries a transaction — e.g. ArriveAtRebin calling
// CreateTask) simply joins the outer scope rather than opening a second
// one, so composing use cases never deadlocks on itself.

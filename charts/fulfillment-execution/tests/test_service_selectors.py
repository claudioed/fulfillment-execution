...
Run: python3 charts/fulfillment-execution/tests/test_service_selectors.py
"""
...
def main() -> int:
    ...
    if "fulfillment-execution" not in services:
        failures.append("the OLTP Service was not rendered")
    elif selector_of(services["fulfillment-execution"]).get("app.kubernetes.io/component") != "api":
        failures.append("the OLTP Service selector must pin component=api")

    if "fulfillment-execution-frontend" not in services:
        failures.append("the frontend Service was not rendered with frontend.enabled=true")
    elif selector_of(services["fulfillment-execution-frontend"]).get("app.kubernetes.io/component") != "frontend":
    ...
    fe = services.get("fulfillment-execution-frontend")

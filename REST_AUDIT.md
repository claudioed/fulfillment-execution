...
| `POST /tasks/{id}/seal-package` | **Yes** (fixed) | `SealPackage.Execute` now checks `ports.PackageRepo.FindByTaskId(taskId)` before creating a `Package`: a retried call for a task that already has a sealed Package returns that existing Package instead of creating a second one. `taskId` is the dedupe key (this endpoint is task-scoped — exactly one Package is ever sealed per Pack task — and `taskId` is already the URL's resource identifier, so no new client-supplied idempotency key was needed). See `internal/domain/package/package.go`'s `TaskId()` field, migration `0011_package_task_id`, and its partial unique index for the database-level guarantee against a race between two concurrent retries. |
